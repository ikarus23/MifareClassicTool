/*
 * Copyright 2013 Gerhard Klostermeier, André Ahlfors Dahl
 *
 ...
 * </ul>
 *
 * @author Gerhard Klostermeier, André Ahlfors Dahl
 */
public class CompareActivity extends BasicActivity {
    ...
    private static final String LOG_TAG =
            CompareActivity.class.getSimpleName();
     ...
     *
     * @see #initEditor(LinearLayout myLayout, String[])
     */
    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_compare_tool);

        mLayout = (LinearLayout) findViewById(
                R.id.LinearLayoutDumpEditor);
        mLayout2 = (LinearLayout) findViewById(
                R.id.LinearLayoutDumpEditor2);
        ...
        if (getIntent().hasExtra("FileOne")) {
            // Called from FileChooser (init editor by file).
            File file = new File(getIntent().getStringExtra(
                    "FileOne"));
            mFileName = file.getName();
            setTitle(getTitle() + " (" + mFileName + ")");
            File file2 = new File(getIntent().getStringExtra(
                    "FileTwo"));
            mFileName2 = file2.getName();
            setTitle(getTitle() + " vs (" + mFileName2 + ")");
            initEditor(mLayout, Common.readFileLineByLine(file, false, this));
            initEditor(mLayout2, Common.readFileLineByLine(file2, false, this));
            setIntent(null);
            ...
            if (mFileName == null) {
                ...
                setTitle(getTitle() + " (" + mFileName + ")" + " (" + mFileName2 + ")");
            }
            mLines = savedInstanceState.getStringArray("lines");
            mLines2 = savedInstanceState.getStringArray("lines2");
            if (mLines != null && mLines2 != null) {
                initEditor(mLayout, mLines);
                initEditor(mLayout2, mLines2);
            }
        }
    }
     ...
     * @see #initEditor(android.widget.LinearLayout, String[])
     */
     ...
     * @see #isValidDump()
     */
    private void initEditor(LinearLayout myLayout, String[] lines) {
        boolean err = false;
    ...
    /*******
     * Below code is for future implementations of the openintents.org support.
     * http://www.openintents.org/en/node/164 - Pick a file
     * http://www.openintents.org/en/node/165 - Pick a directory
     * http://www.openintents.org/en/node/357 - Pick a folder
     * http://www.openintents.org/en/node/916 - Browse file/folder (jump to)
     * http://www.blackmoonit.com/android/filebrowser/intents/ - good resource
     */
    /*
    ...
    void pickFile(File aFile) {
        Intent theIntent = new Intent(FileManagerIntents.ACTION_PICK_FILE);
        theIntent.setData(Uri.fromFile(aFile));  //default file / jump directly to this file/folder
        theIntent.putExtra(Intent.EXTRA_TITLE,"Pick first file"); //optional
        try {
    ...
    /*case PICK_FILE_RESULT_CODE: {
        if (resultCode==RESULT_OK && data!=null && data.getData()!=null) {
            if(theFirstFilePath != null){
                String theFirstFilePath = data.getData().getPath();
            }else if(theSecondFilePath != null){
                String theSecondFilePath = data.getData().getPath();
            }else{
                //We should not be here!
                Toast.makeText(this, "This Should NEVER happen!!!",
                        Toast.LENGTH_SHORT).show();
            }
            //Start handling the both filepaths
        }
        break;
    }*/

    //java.io.File file = new File(Environment.getExternalStoragePublicDirectory(
    //Common.HOME_DIR) +  "/" + Common.DUMPS_DIR);
    //pickFile(file);
    //intent = new Intent(this, CompareActivity.class);
    //intent.putextra (fileinfo here)
    //startActivity(intent);
}
